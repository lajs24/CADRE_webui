...
色标按整个导入集合计算，而不是按单帧或单窗口计算：密度上限是所有窗口中观测、预测和真值的最大物理密度向上取整；误差色标以所有预测帧的最大绝对误差向上取整，并保持正负对称。密度使用连续的白色—黄色—红色顺序色带，零密度显示为画布背景；非线性映射仅增强低至中等密度的可读性，原始 JSON、提示数值和统计指标不受影响。这样切换窗口、帧、真值和预测时，颜色含义不变。
